...
namespace Corecalc {
	...
	/// </summary>
	internal class CellArea : Expr, IEquatable<CellArea> {
		...
		private static void RefAndSupp(bool ulAbs,
									   bool lrAbs,
									   int ra,
									   int rb,
									   int r1,
									   int r2,
									   out Interval referred,
									   out Func<int, Interval> supported) {
			if (ulAbs) {
				if (lrAbs) { // case abs-abs
					...
					supported = r => ra < r
										 ? new Interval(Math.Max(r1, r - rb), r2)
										 : ra > r
											   ? new Interval(r1, Math.Min(r2, r - rb))
											   : new Interval(r1, r2);
				}
			}
			else {
				if (lrAbs) { // case rel-abs
					...
					supported = r => rb > r
										 ? new Interval(r1, Math.Min(r2, r - ra))
										 : rb < r
											   ? new Interval(Math.Max(r1, r - ra), r2)
											   : new Interval(r1, r2);
				}
				else { // case rel-rel
					SortInts(ref ra, ref rb);
		...
		public override void DependsOn(FullCellAddr here, Action<FullCellAddr> dependsOn) { ApplyToFcas(here.sheet, here.ca.col, here.ca.row, dependsOn); }

		public override bool IsVolatile {
			get { return false; }
		}

		public bool Equals(CellArea that) { return that != null && this.ul.Equals(that.ul) && this.lr.Equals(that.lr); }

		public override int GetHashCode() { return lr.GetHashCode()*511 + ul.GetHashCode(); }
