...
namespace CoreCalc.Expressions {
	...
	/// </summary>
	internal class CellRef : Expr, IEquatable<CellRef> {
		...
		public FullCellAddr GetAbsoluteAddr(Sheet sheet, int col, int row) { return new FullCellAddr(this.sheet ?? sheet, raref.Addr(col, row)); }

		public FullCellAddr GetAbsoluteAddr(FullCellAddr fca) { return GetAbsoluteAddr(fca.sheet, fca.ca.col, fca.ca.row); }

		// Clone and move (when the containing formula is moved, not copied!)
		public override Expr Move(int deltaCol, int deltaRow) { return new CellRef(sheet, raref.Move(deltaCol, deltaRow)); }
		...
		private static void RefAndSupp(bool abs,
									   int ra,
									   int r1,
									   int r2,
